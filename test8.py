#Hier die verschiedenen sprechblasen testen
...
def place_eye(frame):
    ...
    x2 = int(left_eyebrow[0] + (right_eyebrow[0] / 2) + (head_width / 2))
    y1 = int(left_eyebrow[1] - (center_eyebrow[1] / 2) - (head_height / 2))
    y2 = int(left_eyebrow[1] + (head_height / 2))
    ...
    eyeOverlay = cv2.resize(imgEye, (head_width, head_height), interpolation=cv2.INTER_AREA)
...
imgEye = cv2.imread('vuforia.png', -1)
